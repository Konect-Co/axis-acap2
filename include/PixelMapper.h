...
namespace PixelMapper {
	...
	vector<Point2f*>* pixel_to_lonlat(PixelMapperConfig &config, std::vector<Point2f*> &pixel_coordinates);

	// Convert a set of lon-lat coordinates to pixel coordinates
	vector<Point2f*>* lonlat_to_pixel(PixelMapperConfig &config, std::vector<Point2f*> &lonlat_coordinates);

	// Convert a set of lon-lat coordinates to 3D coordinates
	vector<Point3f*>* lonlat_to_3D(PixelMapperConfig &config, std::vector<Point2f*> &lonlat_coordinates);

	// Convert a set of 3D coordinates to lon-lat coordinates
	vector<Point2f*>* _3D_to_lonlat(PixelMapperConfig &config, std::vector<Point3f*> &threeD_coordinates);
